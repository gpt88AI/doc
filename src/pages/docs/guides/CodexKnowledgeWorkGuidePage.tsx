...
const WORKSPACE_PROMPT = `我想创建一个 Codex 工作空间。
...
const TASK_PROMPT = `任务：整理本周的客户反馈，生成一份产品改进简报。
...
const REVIEW_PROMPT = `请复盘刚才的任务，只回答以下问题：
1. 你做了哪些关键假设？
2. 哪个决定最难，考虑过哪些替代方案？
3. 哪些事实还没有被原始来源确认？
...
const SEVEN_DAY_PLAN = `第 1 天：列出一周内最重复、最烦、最需要整合信息的任务
...
export default function CodexKnowledgeWorkGuidePage() {
  return (
    <DocPage
      ...
      title="Codex 如何应用于办公和知识工作：从一次任务到可复用系统"
      description="根据 Gorden Sun 的 X Article 整理 Codex 在办公和知识场景中的使用方法，覆盖工作空间、外部工具、审批边界、五级工作流、16 类任务模板和 7 天上手计划。"
      headings={[
        { id: 'source', text: '来源与阅读方式', level: 2 },
        { id: 'conclusion', text: '先看结论', level: 2 },
        { id: 'audience', text: '适合谁，完成标准是什么', level: 2 },
        { id: 'concepts', text: '五个核心概念', level: 2 },
        { id: 'five-step', text: '五步工作法', level: 2 },
        { id: 'workspace', text: '先搭工作空间', level: 2 },
        { id: 'levels', text: '五个使用等级', level: 2 },
        { id: 'shortest-path', text: '最短成功路径：完成一份简报', level: 2 },
        { id: 'workflow-catalog', text: '工作流选择表', level: 2 },
        { id: 'decision', text: '什么时候自动化，什么时候一起做', level: 2 },
        { id: 'review', text: '检查、复盘与沉淀', level: 2 },
        { id: 'risks', text: '常见翻车场景与恢复', level: 2 },
        { id: 'team', text: '团队协作与推广', level: 2 },
        { id: 'seven-days', text: '7 天上手计划', level: 2 },
        { id: 'templates', text: '可复用提示词与模板', level: 2 },
        { id: 'references', text: '来源与证据说明', level: 2 },
      ]}
    >
      <Callout tone="info" title="本文是第三方文章的结构化整理">
        <p>
          原文作者 Gorden Sun 把 Codex 放在办公、调研、写作、跨系统信息整理和自动化的场景里讨论，而不只把它当作写代码工具。本页保留原文的核心框架，同时补充了输入、动作、验证和恢复步骤，方便直接实践。
        </p>
        <p>
          原文中的工具名称、套餐能力、移动端能力和界面入口可能随 Codex 版本、账号类型和连接器状态变化。本文把它们当作工作流示例，不当作当前版本的功能承诺。
        </p>
      </Callout>

      <h2 id="source">来源与阅读方式</h2>
      <p>
      ...
      <p>
        <a href={SOURCE_ARTICLE} target="_blank" rel="noreferrer">
          阅读 X Article 原文
        </a>{' '}
        ·{' '}
        <a href={SOURCE_POST} target="_blank" rel="noreferrer">
          查看 Gorden Sun 的原始状态
        </a>
      </p>
      ...
      <CodeBlock lang="text" filename="codex-work-loop" code="接工具 → 写规则 → 干活 → 检查 → 沉淀" />
      <ul>
        <li><strong>接工具：</strong>让它能访问任务真正需要的资料来源；没有连接器，就先从本地文件和手动提供的资料开始。</li>
        <li><strong>写规则：</strong>把角色、偏好、来源、权限和验收标准放进可复用文件，而不是每次都靠聊天记录重述。</li>
        <li><strong>干活：</strong>明确输入、输出和边界，复杂任务先让 Codex 说明计划。</li>
        <li><strong>检查：</strong>在真实交付位置验证结果，不只在 Codex 聊天窗口里看一遍。</li>
        <li><strong>沉淀：</strong>把有效提示词、错误、决定和验收条件写回工作空间，让下一次更快。</li>
      </ul>

      <h2 id="audience">适合谁，完成标准是什么</h2>
      <p>这篇指南适合以下几类人：</p>
      <ul>
        <li>不以写代码为主要工作，但需要整理资料、写报告、做计划或维护知识库。</li>
        <li>已经使用 Codex，却只把它当作一次性问答或代码补全工具。</li>
        <li>经常在邮件、聊天、文档、表格和本地文件之间搬运信息。</li>
        <li>想把重复工作做成流程，但还没有明确的输入、审批和验收标准。</li>
      </ul>
      <p>完成本文的最小标准不是“学会所有功能”，而是完成一个可检查的低风险任务：</p>
      <ol>
        <li>选定一个真实但不会造成不可逆损失的任务。</li>
        <li>明确它需要读取的两类来源和最终交付格式。</li>
        <li>让 Codex 先说明计划、缺口和检查方式。</li>
        <li>审阅输出，记录一个错误或不确定点。</li>
        <li>把有效步骤保存成工作流文件或检查清单。</li>
      </ol>

      <h2 id="concepts">五个核心概念</h2>
      <p>
        原文用五个概念解释 Codex 的工作结构。不同版本的界面命名可能变化，使用时以当前产品显示为准；概念之间的关系仍然有参考价值。
      </p>
      <SimpleTable
        headers={['概念', '可以怎样理解', '应该保存什么']}
        rows={[
      ...
      />
      <Callout tone="warn" title="不要把对话历史当作唯一数据库">
        <p>
          一个对话里确认过的决定，如果下一次任务仍然需要，就应该写进项目文件、规则文件或工作流文档。新对话不一定继承完整聊天上下文；可恢复性来自文件和明确的状态，而不是“希望模型记得”。
        </p>
      </Callout>

      <h2 id="five-step">五步工作法</h2>
      <SimpleTable
        headers={['阶段', '输入', '动作', '验证信号']}
        rows={[
      ...
      />
      <p>可以用下面的提示词让 Codex 先采访你，再生成初始结构：</p>
      ...
      />
      <Callout tone="tip" title="先把一个低风险任务做稳">
        <p>
          不要因为已经连接了很多工具，就直接进入自动化。先完成一个一次性任务，确认来源、输出、检查和恢复都可控，再升级复杂度。
        </p>
      </Callout>

      <h2 id="shortest-path">最短成功路径：完成一份简报</h2>
      <p>下面用“整理客户反馈”作为练习。它有真实价值，又能在发送、发布和删除动作之前停住。</p>
      <ol>
        <li>
          <strong>准备输入：</strong>选两份已确认的本地会议纪要或导出文件，放入 <code>sources/feedback/</code>，标注日期和来源。
        </li>
        <li>
          <strong>定义输出：</strong>要求生成 Markdown 简报，包含问题分类、影响范围、出现频率、建议动作和待确认问题。
        </li>
        <li>
          <strong>先要计划：</strong>让 Codex 说明要读取哪些文件、缺少什么、如何区分事实和建议。
        </li>
        <li>
          <strong>执行并落盘：</strong>让它把结果写入 <code>workflows/feedback-brief-YYYY-MM-DD.md</code>，不要只返回在聊天窗口。
        </li>
        <li>
          <strong>检查来源：</strong>随机抽查至少 3 个结论，回到原始文件确认语义和日期没有被拼错。
        </li>
        <li>
          <strong>真实验收：</strong>让产品或业务同事阅读简报，确认分类和建议对他们有用，而不是只看格式漂亮。
        </li>
        <li>
          <strong>沉淀：</strong>记录本轮最常见的错误，补进 <code>checklists/feedback-brief.md</code>，再决定是否值得重复使用。
        </li>
      </ol>
      ...
      <h2 id="workflow-catalog">工作流选择表</h2>
      <p>
        原文列出了 16 类起步工作流。下面按风险和输入输出重新分组，便于选择第一项实践，不需要一次全部搭建。
      </p>
      <SimpleTable
        headers={['类别', '工作流示例', '最小输出', '第一道人工检查']}
        rows={[
      ...
      <h2 id="decision">什么时候自动化，什么时候一起做</h2>
      <SimpleTable
        headers={['问题', '倾向完全交给 Codex', '倾向人机一起做']}
        rows={[
      ...
      <h2 id="review">检查、复盘与沉淀</h2>
      <p>原文反复强调“去成果真正使用的地方看”。验证至少分三层：</p>
      <ol>
        <li><strong>事实层：</strong>数字、日期、人物、状态和引用是否能回到原始来源。</li>
        <li><strong>交付层：</strong>格式、结构、链接、文件位置和可读性是否符合实际使用场景。</li>
        <li><strong>风险层：</strong>是否越权发送、删除、发布、修改正式资料，或把外部内容里的恶意指令当成任务。</li>
      </ol>
      <p>
      ...
      <h2 id="risks">常见翻车场景与恢复</h2>
      <SimpleTable
        headers={['问题', '为什么发生', '恢复动作']}
        rows={[
      ...
      />
      <Callout tone="danger" title="高风险动作必须有人工闸门">
        <p>
          不要让 Codex 在没有审批的情况下发送消息、发布内容、删除数据、修改正式资料、做财务操作或处理高敏感信息。最小权限、草稿模式、明确的确认词和可回滚记录，比“让它更自主”更重要。
        </p>
      </Callout>
      <p>
      ...
      <h2 id="team">团队协作与推广</h2>
      <p>
        个人使用顺手后，团队推广的难点不是把所有人一次性培训成高级用户，而是让一个具体的人用一个具体工作流解决一个真实问题。一个可复用的推广顺序是：
      </p>
      <ol>
        <li>选一个重复、低风险、容易验收的任务。</li>
        <li>由一个明确负责人维护输入、权限、规则和检查清单。</li>
        <li>先展示前后对比：节省了什么时间、减少了什么搬运、人工检查保留在哪里。</li>
        <li>把成果放入团队可访问的文档或共享工具，不要只留在个人 Codex 会话里。</li>
        <li>确认稳定后再扩展到更多人和更多来源。</li>
      </ol>
      <p>
        团队文档最好同时满足两件事：人能读懂，Agent 也能按结构读取。每个自动化流程都应该有负责人、输入来源、输出位置、审批边界、验收方法和停用条件。
      </p>

      <h2 id="seven-days">7 天上手计划</h2>
      <p>不要一开始就搭完整的 Agent 系统。按“观察、连接、执行、复用、自动化”的顺序推进：</p>
      <CodeBlock lang="text" filename="seven-day-plan.txt" code={SEVEN_DAY_PLAN} />
      <SimpleTable
        headers={['时间', '当天动作', '当天验收']}
        rows={[
...
失败恢复方式和需要保存到哪个项目文件。不要把未经验证的推断写成规则。`}
      ...
      <ul>
        <li>把散乱笔记整理成结构化提纲。</li>
        <li>从已确认的资料生成会议纪要和待办列表。</li>
        <li>对一篇草稿先做结构、事实和语气诊断，再逐项修改。</li>
        <li>对代码变更生成面向业务人员的影响摘要，但不替代工程审查。</li>
        <li>整理想法库并检查重复，不自动删除被否决的想法。</li>
      </ul>

      <h2 id="references">来源与证据说明</h2>
      <Callout tone="info" title="文章事实与本文建议分开阅读">
        <p>
          原文明确提供了 Codex 办公场景的概念框架、五步工作法、五级路径、工作流示例、团队协作建议和 7 天计划。本文对这些内容做了重排和压缩，并增加了输入 / 输出 / 验证 / 恢复字段，后者属于文档化建议，不是原文逐字内容。
        </p>
        <p>
          原文没有为所有功能提供当前版本的产品文档、权限说明或稳定性承诺。连接器、Goal、Plugin、Site、移动端和团队能力的具体可用性，应以当前 Codex 界面、账号和官方文档为准。
        </p>
      </Callout>
      <ul>
        <li>
        ...
        <li>
          站内延伸：<Link to="/docs/guides/codex-gpt55-system-overview/">Codex + GPT-5.5 实战拆解总览</Link>、{' '}
          <Link to="/docs/guides/codex-skills-context-engineering/">Codex Skills 与上下文工程</Link>、{' '}
          <Link to="/docs/guides/codex-plugins-research-workflow/">Codex 插件与研究工作流</Link>
        </li>
      </ul>
      <Callout tone="tip" title="最终目标">
        <p>
          不要把目标定成“让 AI 替你思考”，而是让 Codex 接手资料搬运、重复整理和可验证的执行步骤，把人的注意力留给判断、品味、策略和最终责任。先做一次真实的小任务，做好检查，再让系统逐步积累。
        </p>
      </Callout>
    </DocPage>
  )
