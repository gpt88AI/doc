...
const FEATURED_DETAILS: Record<string, FeaturedDetail> = {
  'gpt-6-astra': {
    ...
    overview: [
      ...
      '当前公开模型目录尚未返回 gpt-6-astra 的完整实时配置，因此本文档不固化价格、上下文、限速、工具和视觉能力；实际开放范围以 GPT88 控制台和当前 API Key 的模型列表为准。',
      '接入前应先调用 GET /v1/models，确认当前账号确实开放 gpt-6-astra，再使用固定评测样本验证质量、延迟、成本和工具调用表现。',
    ],
    whenToUse: [
      '希望优先测试新一代 GPT 路线能力时',
    ...
    caveats: [
      '当前实时公共模型目录未返回 gpt-6-astra；模型是否可调用、实际模型 ID、价格、限速和能力以 GPT88 控制台为准。',
...
const ENGLISH_MODEL_DETAILS: Partial<Record<string, LocalizedModelDetail>> = {
  'gpt-6-astra': {
    ...
    overview: [
      ...
      'The current public model catalog does not yet expose a complete live configuration for gpt-6-astra, so availability, pricing, context, limits, tools, and vision support remain console-authoritative.',
    ...
    caveats: [
      'The public live catalog does not currently return gpt-6-astra; actual access, pricing, limits, and capabilities are determined by the GPT88 console.',
