...
> 本文根据用户提供的公众号文章 PDF《GPT Images 2.5 首发实测，手绘成图、指哪改哪、也太逼真了！》整理改写，作者署名为“程序员鱼皮”，原文发布日期为 2026 年 9 月 9 日；同时参考了 YouTube 视频[《GPT Image 2.5 到底强在哪？8大场景深度实测｜Open AI｜GPT 6 Codex》](https://www.youtube.com/watch?v=Uhxt39vNWsc)。文中配图来自该 PDF，并保留为站内图片资源。
...
这条视频从“GPT Image 2.5 到底强在哪”出发，采用多场景实测的方式观察新模型，而不是只展示一张漂亮的成图。把这类评测读成自己的测试计划时，可以将场景归并为八类：草图转成图、局部指令编辑、商品与人物一致性、文字排版、复杂构图、风格迁移、透明背景素材，以及多轮连续修改。

每一类任务都应该同时看“目标有没有完成”和“没有要求改变的部分有没有被破坏”。例如，局部编辑不能只看新区域是否漂亮，还要检查人物身份、产品结构、背景和文字是否漂移；商品图不能只看光影，还要核对包装、Logo、颜色和比例；多轮风格转换不能只比较风格差异，还要确认主体是否仍然是同一个对象。

视频页面当前可核验的是标题、频道与公开链接；YouTube 对本次访问要求登录验证，未能取得可复核的完整字幕。因此下文的具体案例、提示词和截图以已整理的 PDF 实测材料为依据，不把视频中可能出现的口播、演示数字或个人评价扩写成未经核实的结论。需要复现时，建议使用相同输入素材和固定提示词重新测试，并记录模型 ID、尺寸、延迟、失败重试和实际扣费。
